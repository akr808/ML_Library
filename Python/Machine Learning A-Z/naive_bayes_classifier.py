...
Topic: Naive Bayes Classifier--Gaussian
@author: Anand
"""
...
plt.contour(X1, X2, nbc.predict(sc_x.transform(np.array([X1.ravel(), X2.ravel()]).T)).reshape(X1.shape), 
            alpha =0.75, cmp = ListedColormap(('red', 'green')))
plt.xlim(X1.min(),X1.max())
plt.ylim(X2.min(),X2.max())
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1], c = ListedColormap(('red', 'green'))(i), label = j)
plt.title("Naive Bayes Classifier-Train")
plt.xlabel("Age")
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1], c = ListedColormap(('red', 'green'))(i), label = j)
plt.title("Naive Bayes Classifier-Test")
plt.xlabel("Age")
plt.ylabel("Estimated Salary") 
plt.legend()
plt.show()   
